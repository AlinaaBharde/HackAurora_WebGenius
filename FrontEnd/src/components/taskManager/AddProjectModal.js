...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`http://localhost:8080/project/${id}`)
                .then((res) => {
                    setTitle(res.data.name)  // Assuming backend sends 'name' instead of 'title'
    ...
    // Handle form submission (create or edit project)
    const handleSubmit = (e) => {
        e.preventDefault();
        ...
        if (!edit) {
            ...
            axios.put(`http://localhost:8080/project/${id}`, projectData)
                .then((res) => {
                    closeModal();
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
